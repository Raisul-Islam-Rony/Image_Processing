...
def calculateErlangValue(x, k, l):
    return ((l ** k) * (x ** (k-1)) * (math.e ** -(l * x))) / (math.factorial(k-1))    # f{x,y} = (l^k * x^k-1 * e^-lx) / (k-1)!

def calculateErlangValue_scale(x, k, miu):
    return ( (x ** (k-1)) * (math.e ** -(x / miu))) / ((miu **k) * math.factorial(k-1)) # f{x,y} = (x^k-1 * e^-x/miu) / miu^k *(k-1)!
